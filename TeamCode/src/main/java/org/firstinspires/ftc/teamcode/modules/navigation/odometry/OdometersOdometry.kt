...
class OdometersOdometry : IRobotModule {
    override fun init(collector: BaseCollector, bus: EventBus) {
        ...
        bus.subscribe(HardwareOdometers.UpdateHardwareOdometersEvent::class) {
            ...
            val deltaY =
                deltaSidePosition - (Configs.OdometryConfig.SIDE_ODOMETER_RADIUS * deltaRotate)
            ...
            if (abs(deltaRotate) < Configs.OdometryConfig.ROTATE_SENS) {
                ...
                deltaXCorrected =
                    deltaX * sin(deltaRotate) / deltaRotate + deltaY * (cos(deltaRotate) - 1.0) / deltaRotate
                deltaYCorrected =
                    deltaX * (1.0 - cos(deltaRotate)) / deltaRotate + deltaY * sin(deltaRotate) / deltaRotate
            }

            _position += Vec2(
                deltaXCorrected, deltaYCorrected
